...
- Use plain `LANGUAGE sql` when the body is a single statement; it inlines into calling queries. Reserve `plpgsql` for control flow.
...
- Keep `SECURITY INVOKER` (the default); use `SECURITY DEFINER` only for a specific privilege boundary, with `search_path` pinned and inputs treated as hostile.
...
- `SECURITY DEFINER` is required for controlled privilege elevation (for example, letting the read-only role refresh one materialized view); isolate, pin `search_path`, and review with [roles, privileges, and row-level security](roles-privileges-and-row-level-security.md).
