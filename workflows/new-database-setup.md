...
5. Apply the grants and `ALTER DEFAULT PRIVILEGES` per the roles guideline — tables, sequences, and `REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC` — and create login roles as members, setting the standing timeouts (`statement_timeout`, `idle_in_transaction_session_timeout`) on each runtime login per the roles guideline; never on `{app}_owner`.
